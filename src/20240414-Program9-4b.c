 ...
 *
 *	lcd_putValue関数でのBSR,BSRRの使用をやめた
 *	lcd_strobe関数を使うと、不具合の状況が再現される。（17:40）
 *
 *	lcd_strobe関数の不具合を修正。
 *	lcd_putValue関数のBSR,BSRRを使う版も動作する。
 *
 *
 *
  */
 ...
 * LCDのストローブ信号の立下りパルス
 *
 * データを確定した後、ストローブ信号の立下りパルスを出せば、
 * LCDモジュールに、そのデータが書き込まれる。
 *
 * 2024/04/14 なぜか、この関数をストローブ信号に使うと、文字が化ける。
 * 不具合に気付いて、修正した。コピペはダメという話！
 *
 */
...
//	delay_us(DELAY);
	...
	GPIOA->ODR&=~0xf0;			//PA7-PA4を０クリア
	GPIOA->ODR|=value&0xf0;	//D7-D4をPA7-PA4に出力する
	delay_us(DELAY);
