/*
 * 20240908-7SegLED_LCD-KeyMatrix-Buz1.c
 *
...
int main(void){
	...
	while(1){
		...
		get_number(&number);	//キーパッドからの入力待ち。*で入力開始、#で入力終了。
		...
		sprintf(message,"[%d] [%d]",number,collatz_count);//Collatz数列の計算結果表示
		lcd_clear_line(1);
		lcd_gotoxy(1,1);
		lcd_print(message);
...
/* 7 セグメント LED 表示パターン用のデータ配列（正論理として定義） */
	...
	if ((number >= 0) && (number <= 9)) {				// 数値が 0 ～ 9 の範囲なら指定された数字のパターンを
		...
		GPIOB->ODR &= ~0x00FF;							// ７セグメント LED を全消灯する
	}
}
#endif
		...
		case INPROGRESS:
			...
			if(determined_key_code=='#'){
				if(digit>0)
					...
					state=INITIAL;
			}else if(determined_key_code=='*'){ //ここに、elseを抜かしていて動作がおかしかった。2024/09/08
				state=INITIAL;
			}else {
